...
from models.ac_models_hier_N4 import CommanderGru
from config import Config
from envs.env_hier import HighLevelEnv

N_OPP_HL = 5 #change for sensing
...
def evaluate(args, algo, env, epoch):
    ...
    def cc_obs(obs):
        return {
            ...
            "obs_4": np.zeros(OBS_DIM, dtype=np.float32),
            "obs_5": np.zeros(OBS_DIM, dtype=np.float32),
    ...
    class CustomCallback(DefaultCallbacks):
        ...
        def on_postprocess_trajectory(
            ...
            **kwargs
            ...
            oth_agents = list(range(1,N_OPP_HL+1))
    ...
    observer_space = spaces.Dict(
        {
            ...
            "obs_4": spaces.Box(low=0, high=1, shape=(OBS_DIM,)),
            "obs_5": spaces.Box(low=0, high=1, shape=(OBS_DIM,)),
            ...
            "act_4": spaces.Box(low=0, high=ACT_DIM-1, shape=(1,), dtype=np.float32),
            "act_5": spaces.Box(low=0, high=ACT_DIM-1, shape=(1,), dtype=np.float32),
        }
    )
