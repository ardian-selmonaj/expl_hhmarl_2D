...
class CommanderGru(RecurrentNetwork, nn.Module):
    ...
    def __init__(self, observation_space, action_space, num_outputs, model_config, name):
        ...
        self.v4 = SlimFC(
            5*(OBS_DIM+1),200, activation_fn= nn.Tanh, initializer=torch.nn.init.orthogonal_,
        )
        self.val_out = SlimFC(
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        self._v34 = torch.cat((input_dict["obs"]["obs_4"], input_dict["obs"]["act_4"]),dim=1)
        self._v35 = torch.cat((input_dict["obs"]["obs_5"], input_dict["obs"]["act_5"]),dim=1)
        self._v4 = torch.cat((self._v1, self._v2, self._v3, self._v34, self._v35), dim=1)
