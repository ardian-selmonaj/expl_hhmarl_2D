...
class RafaleMod(Unit):
    max_deg_sec = 4
    min_speed_knots = 0
    max_speed_knots = 800
    max_knots_sec = 33
    cannon_range_km = 3.0
    cannon_width_deg = 8.5
    cannon_max_time_sec = 350
    cannon_burst_time_sec = 3
    cannon_hit_prob = 0.9
    aircraft_type = 2

    def __init__(self, position: Position, heading: float, speed: float, group:str, friendly_check: bool = True):
        super().__init__("RafaleMod", position, heading, speed)
        self.new_heading = heading
        self.new_speed = speed
        self.max_speed = RafaleMod.max_speed_knots

        # Cannon
        self.cannon_remain_secs = RafaleMod.cannon_max_time_sec
        self.cannon_current_burst_secs = 0
        self.cannon_max = RafaleMod.cannon_max_time_sec
        ...
        self.group = group
        self.ac_type = RafaleMod.aircraft_type

    def set_heading(self, new_heading: float):
        if new_heading >= 360 or new_heading < 0:
            raise Exception(f"RafaleMod.set_heading Heading must be in [0, 360), got {new_heading}")
        self.new_heading = new_heading

    def set_speed(self, new_speed: float):
        if new_speed > RafaleMod.max_speed_knots or new_speed < RafaleMod.min_speed_knots:
            raise Exception(f"RafaleMod.set_speed Speed must be in [{RafaleMod.min_speed_knots}, {RafaleMod.max_speed_knots}] "
                            f"knots, got {new_speed}")
        self.new_speed = new_speed

    def fire_cannon(self):
        self.cannon_current_burst_secs = min(self.cannon_remain_secs, RafaleMod.cannon_burst_time_sec)
    ...
    def update(self, tick_secs: float, sim: CmanoSimulator) -> List[Event]:
        ...
        if self.heading != self.new_heading:
            ...
            max_deg = RafaleMod.max_deg_sec * tick_secs
        ...
        if self.speed != self.new_speed:
            ...
            max_delta = RafaleMod.max_knots_sec * tick_secs
                ...
                if unit.id != self.id:
                    if unit.id <= sim.num_units + sim.num_opp_units: # consider only aircrafts, no rockets
                        if self.friendly_check or (self.group == "agent" and unit.id >= sim.num_units+1) or (self.group == "opp" and unit.id <= sim.num_units):
                            if unit.type in ["RafaleMod", "Rafale"]:
                                if self._unit_in_cannon_range(unit):
                                    if sim.rnd_gen.random() < \
                                            (RafaleMod.cannon_hit_prob / (RafaleMod.cannon_burst_time_sec / tick_secs)):
    ...
    def _unit_in_cannon_range(self, u: Unit) -> bool:
        ...
        if distance < RafaleMod.cannon_range_km:
            bearing = units_bearing(self, u)
            delta = abs(signed_heading_diff(self.heading, bearing))
            return delta <= RafaleMod.cannon_width_deg / 2.0
        else:
            return False
